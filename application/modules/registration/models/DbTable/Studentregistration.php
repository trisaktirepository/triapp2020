...
class Registration_Model_DbTable_Studentregistration extends Zend_Db_Table_Abstract {
    ...
    protected $_referenceMap    = array(
        'StudentProfile' => array(
            'columns'           => 'IdApplication',
            'refTableClass'     => 'Records_Model_DbTable_Studentprofile',
            'refColumns'        => 'appl_id'
        ),
        'Program' => array(
            'columns'           => 'IdProgram',
            'refTableClass'     => 'Registration_Model_DbTable_Program',
            'refColumns'        => 'IdProgram'
        )
    );
	...
	public function getData($id) {
		...
		->joinLeft(array('g' => 'student_profile'),'g.appl_id=a.IdApplication',array('StudentName'=>'CONCAT(appl_fname," ",appl_lname)'))
...
//			return $result;
    ...
    public function fetchStudentSemesterHistory($lintidreg) {
        ...
        $sql = $db->select()
                        ...
                        ->joinLeft(array('e' => 'tbl_semestermaster'), 'e.IdSemesterMaster=a.IdSemesterMain', array('e.SemesterMainName', 'e.SemesterMainCode','e.SemesterCountType'))
                        //->join(array('c' => 'tbl_user'), 'c.iduser=a.UpdUser', array("c.loginName"))
                        ->joinLeft(array('deftn' => 'tbl_definationms'), 'deftn.idDefinition=a.studentsemesterstatus', array('deftn.DefinitionCode'))
                        ->join (array("ay" => "tbl_academic_year"),'ay.ay_id=e.idacadyear',array("ay_code"))
                        ->where('a.IdStudentRegistration = ?', $lintidreg)
                        ->where('a.studentsemesterstatus <> 456')
                        ->order('ay_code')
                        ->order('SemesterCountType')
                        ->order('SemesterFunctionType');
        $result = $db->fetchAll($sql);
        return $result;
    }
    ...
    public function getStudentRegistrationDetail($id) {
        $sql = $this->lobjDbAdpt->select()
                        ->from(array('a' => 'tbl_studentregistration'), array('a.registrationId', 'a.IdProgram', "CONCAT_WS(' ',IFNULL(a.FName,''),IFNULL(a.MName,''),IFNULL(a.LName,'')) AS name", "a.ExtraIdField1 AS NRIC", "a.IdProgram", "a.IdIntake", "a.profileStatus", 'a.IdStudentRegistration', 'a.Gender', 'a.HomePhone', 'a.CellPhone'))
                        ->joinLeft(array('b' => 'tbl_program'), "a.IdProgram = b.IdProgram", array("b.ProgramName", "b.IdScheme"))
...
/*
	...
	public function getListStudent($post=NULL,$IdSemester=NULL) {
        ...
        if($IdSemester){
        	$lstrSelect->join(array("sem"=>"tbl_studentsemesterstatus"),'sa.IdStudentRegistration=sem.IdStudentRegistration');
        	$lstrSelect->join(array("semd"=>"tbl_semestermaster"),'sem.IdSemesterMain=semd.IdSemesterMaster');
    ...
    /**
     * Function to get senior student next semester approval
     * @author Jasdy
     */
    public function fetchStudentSemesterSenior($lintidreg) {
        $db = Zend_Db_Table::getDefaultAdapter();
        $sql = $db->select()
        ->from(array('a' => 'tbl_studentsemesterstatus'), array('a.*'))
        ->joinLeft(array('d' => 'tbl_semester'), 'd.IdSemester=a.idSemester', array('d.SemesterCode'))
	...
	public function getInfo($registrationId) {
        $db = Zend_Db_Table::getDefaultAdapter();
        $sql = $db->select()
    ...
    public function SearchStudent($data=null){
    	    	
    	$db = Zend_Db_Table::getDefaultAdapter();
    	
        $sql = $db->select()
                      	->from(array('sr' => 'tbl_studentregistration'),array('registrationId','IdStudentRegistration','IdProgram','IdLandscape','IdProgramMajoring','IdIntake')) 
                        ...
                        ->join(array('sp'=>'student_profile'),'sp.appl_id=sr.IdApplication',array('student_name'=>"CONCAT_WS(' ',appl_fname,appl_mname,appl_lname)"))
                        ...
                        if(isset($data)){
                        	...
                        	if(isset($data['student_name']) && $data['student_name']!=''){
                        		$sql->where('sp.appl_fname like %?%', $data['stundet_name']);
                        	}	
                        }
    ...
    public function SearchStudentRegistration($data=null){
    
    	$db = Zend_Db_Table::getDefaultAdapter();
    	 
    	$sql = $db->select()
    	->from(array('sr' => 'tbl_studentregistration'),array('registrationId','IdStudentRegistration','IdProgram','IdLandscape','IdProgramMajoring'))
    	//->join(array('sss'=>'tbl_studentsemesterstatus'),'sss.IdStudentRegistration=sr.IdStudentRegistration',array('idstudentsemsterstatus','Level'))
