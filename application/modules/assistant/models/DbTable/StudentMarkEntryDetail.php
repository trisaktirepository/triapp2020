...
class Assistant_Model_DbTable_StudentMarkEntryDetail extends Zend_Db_Table { //Model Class for Users Details
	...
	public function getIdRegSubjects($idsemester,$idstd,$idsubject){
		$db = Zend_Db_Table::getDefaultAdapter();
		...
		return $row['IdStudentRegSubjects'];
	}
	
	public function updateData($data,$id){
		 $db = Zend_Db_Table::getDefaultAdapter();
		 $db->update($this->_name,$data, "IdStudentMarkDetail = '".(int)$id."'");
	}
	
	
	public function checkMarkEntry($IdStudentRegistration,$IdMarksDistributionDetail,$idSemester,$IdMarksDistribution=null){
		 ...
		 //check ada x mark entry sebelum ni
	 	 ...
	 	 if ($IdMarksDistribution!=null) $select_mark->where('sme.IdMarksDistributionMaster=?',$IdMarksDistribution);
	...
	public function getMark($IdStudentRegistration,$idmarkdetail,$idsemester){
		 ...
		 //check ada x mark entry sebelum ni
	 	 $select_mark = $db->select()
	 	 				  ...
	 	 				  ->where('sme.IdStudentRegistration = ?',$IdStudentRegistration)
	 	 				  ->where('sme.IdMarksDistributionDetail = ?',$idmarkdetail) 
	 	 				  ->where('sme.IdSemester = ?',$idsemester);
	...
	public function getStudentTotalMarkPerMhs($idmark,$IdStudentRegistration,$remark=false){
		...
		$dbMarkDist=new App_Model_Exam_DbTable_Marksdistributionmaster();
		$main_component=$dbMarkDist->getInfoComponent($idmark);
		$markpercen=$markfinal*$main_component['Percentage']/100;
		
		//get KRS
		$idsubject=$main_component['IdCourse'];
		$idSemester=$main_component['semester'];
		$idStudentRegSubjects=$this->getIdRegSubjects($idSemester, $IdStudentRegistration, $idsubject);
		//save dalam studentregsubject
		$subRegDB = new Assistant_Model_DbTable_StudentMarkEntry();
		$markmaster=$subRegDB->getStudentMarkByMarkDistributionMaster($idmark, $IdStudentRegistration);
		if ($markmaster) {
			//update mark total 
			$data=array('TotalMarkObtained'=>$markfinal,'FinalTotalMarkObtained'=>$markpercen,'IdStudentRegSubjects'=> $idStudentRegSubjects);
		...
		return array('markall'=>$markfinal,'markallfinal'=>$markpercen);
	}
	...
	public function fnGetMarksDistributionDetailHeaderByMaster($idMaster,$entrier=null) { //Function for the view user
		...
		->from(array("a" => 'tbl_markdistributiondetail_header'))
	...
	public function getDataProgram($idstd){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
			...
			$programs= $db->select()
			->from(array('cgt'=>'tbl_course_tagging_group_assistant'),array())
			->join(array('cgp'=>'course_group_program_assistant'),'cgt.IdCourseTaggingGroup=cgp.group_id',array('cgp.program_id'))
			->join(array('p'=>'tbl_program'),'p.IdProgram=cgp.program_id')
			->where('cgt.IdLecturer=?',$idstd)
			->group('p.IdProgram'); 
	 
		$row = $db->fetchAll($programs);
		return $row;
	}
