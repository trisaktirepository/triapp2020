...
class Onapp_Model_DbTable_Course extends Zend_Db_Table_Abstract {
	/**
	 * The default table name 
	 */
	protected $_name = 'program';
	protected $_tablejoin = 'masterprogram';
	...
	public function getPaginateData($cond){
		...
		$select = $db->select()
					->from($this->_name)
					->join($this->_tablejoin, 'program_master_id = masterProgramID')
					->where('program_award_id  ='.$cond);
	...
	public function getList($table,$tbljoin,$joincond,$tbljoin2,$joincond2,$cond){
		$db = Zend_Db_Table::getDefaultAdapter();
