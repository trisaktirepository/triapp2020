<?php
class Examination_Model_DbTable_Academicstatus extends Zend_Db_Table_Abstract { //Model Class for Users Details
	...
	public function getAcademicStatus($idSemester,$idProgram,$type=0,$basedon='Program',$point_obtained){
		...
		$lobjDbAdpt =  Zend_Db_Table::getDefaultAdapter();
		$point_obtained=number_format($point_obtained, 2, '.', '');
		...
		if(!$larrResult){
			...
			$row=$lobjDbAdpt->fetchRow($select_grade);
			$dt=$row['SemesterMainStartDate'];
			...
			$select_grade = $lobjDbAdpt->select()
									...
									->where('a.AcademicStatus =? OR a.AcademicStatus = 2',$type)
									->where('a.BasedOn =?',$basedon)
									->where('sm.SemesterMainStartDate <=?',$dt)
									->where("a.Active = ?", 1)
	...
	public function getListAcademicStatus($idSemester,$idProgram,$type=0,$basedon='Program'){
		...
		if(!$larrResult){
			...
			$select_grade = $lobjDbAdpt->select()
			->from(array("a" => "tbl_academicstatus"),array())
			->join(array('ad'=>'tbl_academicstatus_details'),'ad.IdAcademicStatus=a.IdAcademicStatus',array())
			->join(array('sm'=>'tbl_semestermaster'),'sm.IdSemesterMaster=a.IdSemester',array('IdSemesterMaster','SemesterMainStartDate'))
			->where('a.IdProgram =?',$idProgram)
			->where('a.AcademicStatus =? OR a.AcademicStatus = 2',$type)
			->where('a.BasedOn =?',$basedon)
			->where("a.Active = ?", 1)
			...
			if ($larrResult) $idSemester=$larrResult['IdSemesterMaster'];
