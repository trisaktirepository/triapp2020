...
class Examination_Model_DbTable_Studentassessment extends Zend_Db_Table_Abstract { //Model Class for Users Details
	...
	public function fncalculatestudentcourseGpa($data){
		...
		foreach($stmarksdist as $ret){
			 $studentmarks = $this->fngetstudentCoursemarks($data['IdStudentRegistration'],$data['IdStudentRegSubjects'],$ret['IdComponentItem'],$ret['IdCourse'],$ret['semester'],$ret['IdComponentType']);
			 if(!empty($studentmarks)){
			 	foreach($studentmarks as $record){
			 		//echo floatval($studentmarks['TotalMarkObtained']);
			 		$marksobtained = $marksobtained + floatval($record['TotalMarkObtained']);
			 		$totalmarks = $totalmarks + floatval($record['MarksTotal']);
			 		$avmark = (floatval($record['TotalMarkObtained'])) / (floatval($record['MarksTotal']));
			 		$marks = $marks + ($avmark * $ret['Percentage']);
			 	}
			 }
			 $ret['TotalMarkObtained'] = $marksobtained;
			 $ret['MarksTotal'] = $totalmarks;
			 $ret['getmarks'] = $marks;
	...
	public function fngetstudentCoursemarks($Idstudent,$idstudentregsubject,$componentitem,$course,$semcode,$componenttype){
		$lstrSelect = $this->lobjDbAdpt->select()
						...
						->where("a.Component =?",$componenttype)
	...
	public function fngetsetudentcoursemarks($Idstudent,$idstudentregsubject){
		$lstrSelect = $this->lobjDbAdpt->select()
						->from(array('a' => 'tbl_studentregsubjects'), array('a.Grade','a.Totalmarks','a.GradePoint'))
						->Join(array('b' => 'tbl_definationms'), 'a.Grade=b.idDefinition',array('b.DefinitionDesc as IdGrade'))								
						->where("a.IdStudentRegistration = ?",$Idstudent)
						->where("a.IdStudentRegSubjects =?",$idstudentregsubject);
		$larrResult = $this->lobjDbAdpt->fetchRow($lstrSelect);	
		return $larrResult;
