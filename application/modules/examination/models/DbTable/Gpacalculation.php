...
class Examination_Model_DbTable_Gpacalculation extends Zend_Db_Table_Abstract
{
	...
	public function UpdateCgpa($newcgpa,$idregistration,$lintIdapplication){
		...
		$lstrWhere = "IdRegistration = ".$idregistration." AND IdApplication=".$lintIdapplication;
	...
	public function fnGetSubjectprerequisitsvalidation($lintidsubject,$lintidapplicant,$takemarks){
		...
		if($takemarks == 0) {
			...
			$select ='SELECT w.*,SUM(w.totalsubmarks) as fullcalculatedmarks FROM
					(SELECT `c`.`IdSemestersyllabus`,`sa`.`IDCourse`,`e`.`idVerifierMarks`, `e`.`idverifier` , AVG(e.verifiresubjectmarks) AS `totalsubmarks`,`sm`.`IdSubject` ,`sm`.`SubjectName`,`sm`.`CreditHours`,`gpa`.`Idgpacalculation`
					...
					INNER JOIN `tbl_verifiermarks` AS `e` ON d.idSubjectMarksEntry = e.idSubjectMarksEntry AND (c.IdStudentRegistration  =  '.$lintidapplicant.')
