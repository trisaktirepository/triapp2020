<?php
class Examination_Model_DbTable_CourseGroupStudentAttendanceDetail extends Zend_Db_Table_Abstract {
	...
	public function getDetailData($att_head_id){
		...
		$selectData = $db->select()
				 ->from(array('cgad'=>$this->_name),array('statushadir'=>'cgad.status','cgad.*'))
	...
	public function getAttendanceStatusSessionCount($sesiid,$status){
	...
	public function getAttendanceStatusCountPerSession($group_id,$student_id,$status,$attid){
		...
		->where('cga.group_id=?',$group_id)
		->where('cgad.student_id=?',$student_id)
		->where('cga.class_date<=?',$dateattendance)
		->where('cgad.status=?',$status);
	...
	public function getAttendanceStatus($id,$student_id){
	
		$db = Zend_Db_Table::getDefaultAdapter();
		$selectData = $db->select()
		->from(array('cgad'=>$this->_name))
		->where('cgad.course_group_att_id=?',$id)
		->where('cgad.student_id=?',$student_id);
		$row = $db->fetchRow($selectData);
