...
class GeneralSetup_Model_DbTable_Program extends Zend_Db_Table_Abstract {
 ...
 public function fngetProgramData($program_id=0) { //Function to get the user details
  ...
  public function fnviewProgramMajoring($IdProgram) { // Function to edit Purchase order details
    $select = $this->select()
                    ->setIntegrityCheck(false)
                    ->join(array('a' => 'tbl_programmajoring'), array('a.*'))
                    ->where("a.idProgram  = '$IdProgram'");
    //echo $select;die();
    $result = $this->fetchAll($select);
    return $result;
...
public function getProgramDetails() {
	...
	public function getPaginateProgramDetails($formdata=null) {
    	...
    	if($formdata!=''){
    		$lstrSelect->where("a.ProgramName LIKE '%".$formdata["keywords"]."%'");
    		$lstrSelect->orwhere("a.ArabicName LIKE '%".$formdata["keywords"]."%'");
    		$lstrSelect->orwhere ("a.ProgramCode LIKE '%".$formdata["keywords"]."%'");
    	}
  ...
  public function getProgramDataByCode($progcode=0) { //Function to get the user details
  	 
  	if($progcode!=0){
  		$lstrSelect = $this->lobjDbAdpt->select()
  		->from(array("a" => "tbl_program"))
  		->where("a.ProgramCode =?", $progcode);
