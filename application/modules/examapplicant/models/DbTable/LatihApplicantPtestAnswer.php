...
class Examapplicant_Model_DbTable_LatihApplicantPtestAnswer extends Zend_Db_Table_Abstract {
	/**
	 * The default table name 
	 */
	protected $_name = 'latih_applicant_ptest_ans';
	...
	public function getFirstQuestion($apaid,$compid){
		...
		->join(array('b'=>'latih_applicant_ptest_ans_detl'),'a.apa_id=b.apad_apa_id',array('apad_ques_no'))
		->join(array('c'=>'tbl_question_bank'),'b.idQuestion=c.idQuestion',array())
		->where('a.apa_id = '.$apaid)
		->where('c.subject=?',$compid)
	...
	public function addData($postData){
		...
		$sql ="select * from latih_applicant_ptest_ans where apa_trans_id ='".$postData['apa_trans_id']."' and pcode ='".$postData['pcode']."' and test_type='".$postData['test_type']."'";
		$row = $db->fetchRow($sql);
		 
		$sqld ="delete from latih_applicant_ptest_ans where apa_id='".$row["apa_id"]."'";
		$db->query($sqld);
		$sqld2 ="delete from latih_applicant_ptest_ans_detl where apad_apa_id='".$row["apa_id"]."'";
		$db->query($sqld2);
		$sqld3="delete from latih_applicant_ptest_comp_mark where apcm_at_trans_id='".$postData['apa_trans_id']."' and pcode ='".$postData['pcode']."' and apcm_apa_id='".$row['apa_id']."'";
		$db->query($sqld3);
		$db->beginTransaction();
		  		...
		  		foreach ($compset as $comporder) {
		  			...
		  			foreach ($questionset as $quest) {
			  			...
			  			if ($j<$data[$comporder]['n_question']) {
			  				$db->insert('latih_applicant_ptest_ans_detl',$dtl_data);
			  				$i++;
			  				$j++;
			  			}
