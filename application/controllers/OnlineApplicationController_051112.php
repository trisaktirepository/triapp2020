    	...
    	if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid($formData)) {
                ...
                if ($result->isValid()) {
                    ...
                    $transaction_data= $transDB->getData($data->appl_id); 
                    ...
                    if($transaction_data["at_status"]=='CLOSE' || $transaction_data["at_status"]=='PROCESS'){
...
public function ajaxGetProgrammePtAction(){
	...
	public function uploaddocumentAction(){
    	...
    	$getUploadData = $uploadFileDB->getUploadData($appl_id);
	...
	public function uploaddocumentfilesAction(){
    	...
    	if ($this->getRequest ()->isPost ()) {
    		...
    		if($appl_id==$formData["appl_id"]){
				...
				///upload_file
				$major_path = DOCUMENT_PATH."/".$formData["appl_id"];
...
//    		if ($formData["appl_admission_type"]==2) {
//    			$this->_redirect($this->view->url(array('module'=>'default','controller'=>'online-application','action'=>'placement-test'),'default',true));
//    		}else if ($formData["appl_admission_type"]==1){
...
//    		}
	...
	public function placementTestAction() {
    	...
    	'aphfeeslocation'=>$program["aph_fees_location"],'transactionid'=>$transaction_id));
	...
	public function confirmationAction(){
    	...
    	//get applicant profile
    	$appProfileDB  = new App_Model_Application_DbTable_ApplicantProfile();	
    	$applicant = $appProfileDB->getProfile($appl_id);
    	...
    	$photo = $fileDB->getFile($appl_id,33); //photo
    	$nric = $fileDB->getFile($appl_id,34); //nric
    	if($admission_type==2){
    		$raport = $fileDB->getFile($appl_id,36); //Raport Halaman Identitas, Halaman nilai Kelas 1 dan 3, serta 3 (jika ada) 
		}else{
			$raport="x";
		}   	
    	$transcript = $fileDB->getFile($appl_id,37); //transcript
    	
		if((!$photo) || (!$nric) || (!$transcript) || (!$raport)){
    	...
    	if ($this->getRequest()->isPost()) {
    		...
    		$formData = $this->getRequest()->getPost();
			       ...
			       if($admission_type==1){
				    		...
				    		$upddata["at_submit_date"]=date("d-m-Y");
							...
							$fieldValues = array (
								 ...
								 'Schoolname' => $applicant["education"],//jurusan yg dipilih
							     'TestDate' => date('M j Y',strtotime($placement["aps_test_date"])),
			       ...
			       if($admission_type==2){		
			       	 	    ...
			       	 	    //template path
			       			$filepath = DOCUMENT_PATH."/template/validasi_bank_hs.docx"; 
			       			
			       			//filename
							$output_filename = $applicantID."_validasi_bank_hs.pdf";
			       			 
    						//get academic year
				    	    $ayearDb = new App_Model_Record_DbTable_AcademicYear();
							$academic_year = $ayearDb->getCurrentAcademicYearData();
							...
							$scheduleDB = new App_Model_Record_DbTable_SemesterActivitySchedule();
							
						
							$condition1=array('id'=>3,'sem_id'=>$semester["sm_id"]);//$seleksi
							$seleksi = $scheduleDB->getInfo($condition1);
							
							$condition2=array('id'=>4,'sem_id'=>$semester["sm_id"]);//registrasi
							$registrasi = $scheduleDB->getInfo($condition2);
							
							$condition3=array('id'=>5,'sem_id'=>$semester["sm_id"]);//$withdrawal
							$withdrawal = $scheduleDB->getInfo($condition3);
							...
							$fieldValues = array (
				        	    ...
				        	    'email' => $applicant["appl_email"],
				        	    'Discipline' => $applicant["education"] ,
				        		'PROGRAM1' => $program_data["program_name1"],
						     	'PROGRAM2' => $program_data["program_name2"],
				        	    'submission_date'=>date('M j Y'),
				        	    'ACADEMICYEAR'=>$academic_year["ay_code"],
							 	'registration_date'=>date('M j Y'),
				        	    'withdrawal_date'=>date('M j Y'),
				        	    'seleksi_date'=>$seleksi["sas_start_date"],
							    'registration_date'=>$registrasi["sas_start_date"],
							    'withdrawal_date'=>$withdrawal["sas_start_date"]
				        	    
				        	);
							        	
							        	
				        $educationDB = new App_Model_Application_DbTable_ApplicantEducation();
				    	$a = $educationDB->getEducationDetail($appl_id);
			       ...
			       // ------- create PDF File section	--------	
					...
					$output_directory_path = DOCUMENT_PATH."/".$appl_id;			
					...
					$doc["ad_appl_id"]=$appl_id;
					$doc["ad_type"]=32; //validasi bank
...
/*			print_r($fieldValues);
	...
	public function verificationAction(){
    	...
    	if($admission_type==1){
			...
			if($rstrans["at_status"]=="PROCESS"){
				$this->_redirect($this->view->url(array('module'=>'default','controller'=>'online-application', 'action'=>'viewvalidasi'),'default',true));
			}
		}
	...
	public function verificationyAction(){
...
public function verificationxAction(){
    	...
    	if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid($formData)) {
					...
					if($applicant){
							   ...
							   $this->view->title = $this->view->translate("placement_test_card");
						       ...
						       //--------get applicant program  -----------
						    	...
						    	//echo '<br>'.$program_data["program_code1"];
						    	//echo '<br>'.$program_data["program_code2"];
						    	
						    	     
						    	
					    	    //------to call procedure to get sit no-----
					    		$appprogramDB = new App_Model_Application_DbTable_ApplicantProgram();
						    	$data = $appprogramDB->getProcedure($program_data["program_code1"],$program_data["program_code2"]);	
						    							    	
						    	//Array ( [0] => Array ( [@vRoomId] => 262 [@vSitNo] => AC 406-012 ) ) 
						    	$location='';
						    	...
						    	if($data){
							    	...
							    	/*  update database.table appl_ptest.*/								
							    	
							    	$ptest_info["apt_room_id"] = $roomid;
							    	$ptest_info["apt_sit_no"]  = $sitno;
							    	$location = $ptest_info["apt_sit_no"];							    	 
							    	
							    	$ptestDB = new App_Model_Application_DbTable_ApplicantPtest();
							    	$ptestDB->updateInfo($ptest_info,$transaction_id);
						    	}//end update
							   						    	
						    	
							    //------to ccheck document laready exi-------st	
						    	$documentDB = new App_Model_Application_DbTable_ApplicantDocument();
						    	$document = $documentDB->getData($appl_id,30); //kartu
						    	...
						    	if(!$document){
						    		    ...
						    		    // $photo = $photoDB->getFile($appl_id,33); //PHoto
						    		    ...
						    		    $componentDB = new App_Model_Application_DbTable_ApplicantProgram();
						    		   	$rs_component = $componentDB->getComponentSchedule($transaction_id);	
						    			//print_r($rs_component);
						    			
						    			$x=1;
						    			foreach($rs_component as $component){
											$comp['comp_program'.$x]=$component["ac_comp_name"];
											$comp['exam_date_time'.$x]=date('M d Y',$component["aps_test_date"]).' '.$component["aps_start_time"];											
						    			$x++;
						    			}
						    												    	
										
										$fieldValues = array (
										     'pinnumber' => $billing_no, 
										 	 'Applicantname' => $applicant["appl_fname"].' '.$applicant["appl_mname"].' '.$applicant["appl_lname"],
										     'Address1' => $applicant["appl_address1"],
										     'Address2' => $applicant["appl_address2"],
											 'Mobilenumber' => $applicant["appl_phone_hp"],
										...
										//filename
										$output_filename = $billing_no."_kartu.pdf";
						    		    ...
						    		    $this->view->download_file = "http://".APP_HOSTNAME."/documents/".$appl_id."/".$document["ad_filename"];
						    	}	
						 ...
						 $this->view->noticeMessage = $this->translate("invalid_pass_login");
					}
			}
    	}else{
    		$this->view->form = $form;
	...
	public function registerAction() {
    	...
    	if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid($formData)) {
				...
				$data["at_create_date"]=date("d-m-Y");				
				...
				$this->view->form = null;
    			$this->view->notice = $this->view->translate('please_check_email')."<br />". $this->view->translate('thank_you');
				}
...
/*			$scheddb=new App_Model_Application_DbTable_ApplicantPlacementSchedule();
	    	...
	    	if(!$document){
    		    ...
    		    if($photo["auf_file_name"]){
    		   	...
    		   	//------get program component info------
    		    $componentDB = new App_Model_Application_DbTable_ApplicantProgram();
    		   	$rs_component = $componentDB->getComponentSchedulebytype($transaction_id,1);	
    			...
    			foreach($rs_component as $component){
    				...
    				$comp['location_venue_sitno1'] = "KAMP ".$applicant["apt_sit_no"];
					//$x++;
    			}
    			
    			$rs_component2 = $componentDB->getComponentSchedulebytype($transaction_id,2);									    	
		    	if($rs_component2){
	    			foreach($rs_component2 as $component2){
						$comp['comp_program2'] .= $component2["ac_comp_name"].", ";
						$comp['exam_date_time2']=date('M d Y',$component2["aps_test_date"]).' '.$component2["aps_start_time"];											
	    				$comp['location_venue_sitno2'] = "KAMP ".$applicant["apt_sit_no"];
						//$x++;
	    			}	
		    	}	
		    	//print_r($comp);exit;		
			...
			//----------SURAT undangan orang tua section------
	    		...
	    		$surat = $suratDB->getData($appl_id,42); //kartu
	    	    $surat=false;
	    		...
	    		if(!$surat){
				    	...
				    	//create directory to locate file
						$output_directory_path_surat = DOCUMENT_PATH ."/".$appl_id;
						...
						$doc["ad_filepath"]=$output_directory_path_surat;
						$doc["ad_filename"]=$output_filename_surat;
						$doc["ad_createddt"]=date("Y-m-d");
						$documentDB->addData($doc);	
						
						$this->view->download_surat_undangan = "http://".APP_HOSTNAME."/documents/".$appl_id."/".$output_filename_surat;
	    		}else{
	    			    $this->view->download_surat_undangan = "http://".APP_HOSTNAME."/documents/".$appl_id."/".$surat["ad_filename"];
	    		}
				//-----------------end SURAT undangan orang tua section --------------------
		}
    }
    
   public function viewkartuxAction(){
...
/*			$scheddb=new App_Model_Application_DbTable_ApplicantPlacementSchedule();
	    	...
	    	if(!$document){
    		    ...
    		    $photoDB = new App_Model_Application_DbTable_UploadFile();
    		    $photo = $photoDB->getFile($appl_id,33); //PHoto
    		    $photo_name =  DOCUMENT_PATH."/".$appl_id."/".$photo["auf_file_name"]; //get directory DOCUMENT_PATH/$appl_id/$photo_name
				...
				$fieldValues = array (
				     ...
				     'username' => $applicant["appl_email"],
				     'password' => $applicant["appl_password"],
					'image:photo' => $photo["auf_file_name"]
				);
    ...
    public function viewvalidasiAction(){
		
		$this->view->title = $this->view->translate("view_validasi");		
