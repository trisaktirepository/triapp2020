...
//						
	...
	public function removeFileAction(){
    	...
    	///upload_file
		$major_path = DOCUMENT_PATH."/".$transaction_id;
    	...
    	if ($this->getRequest ()->isPost ()) {
			...
			for ($i=0; $i<$chk; $i++){
				...
				$getfile = $uploadFileDB->getFileByID($appl_id,$fileid);
    	...
    	if ($this->getRequest()->isPost()) {
    		...
    		$formData = $this->getRequest()->getPost();
			       ...
			       if($admission_type==1){
				    		...
				    		$upddata["at_submit_date"]=date("Y-m-d");
			       ...
			       if($admission_type==2){		
							...
							$fieldValues = array (
				        	    ...
				        	    'seleksi_date'=>$seleksi["StartDate"].' s.d '.$seleksi["EndDate"],
...
/*			$scheddb=new App_Model_Application_DbTable_ApplicantPlacementSchedule();
	    	...
	    	if(!$document){
			    ...
			    $this->view->download_file = "http://".APP_HOSTNAME."/documents/applicant/".date("mY")."/".$transaction_id."/".$document["ad_filename"];
			...
			//----------SURAT undangan orang tua section------
	    		...
	    		if(!$surat){
	    			    ...
	    			    $this->view->download_surat_undangan = "http://".APP_HOSTNAME."/documents/applicant/".date("mY")."/".$transaction_id."/".$surat["ad_filename"];
	    		}
    ...
    public function viewletterAction(){
    	...
    	$documentDB = new App_Model_Application_DbTable_ApplicantDocument();
    	$document = $documentDB->getData($appl_id,32); //kartu/confirmation letter
