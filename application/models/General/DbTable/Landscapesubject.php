<?php
class App_Model_General_DbTable_Landscapesubject extends Zend_Db_Table_Abstract
{
    ...
    public function getPrerequisiteCourse($idLandscape,$idSubject,$idprogram,$idProgramMajoring=null){
    	...
    	->join(array("sp" =>"tbl_subjectprerequisites"),'sp.IdLandscapeSub=ls.IdLandscapeSub')
    	->join(array('ln'=>'tbl_landscape'),'ln.Idlandscape=ls.IdLandscape')
    	->where("ls.IdLandscape = ?",$idLandscape)
    	->where("ls.IdSubject = ?",$idSubject)
    	->where('ln.IdProgram=?',$idprogram);
