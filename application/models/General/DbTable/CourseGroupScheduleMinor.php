...
class App_Model_General_DbTable_CourseGroupScheduleMinor extends Zend_Db_Table_Abstract {
	...
	public function isSchedule($idGroup,$date,$start,$stop){
		...
		->where('idGroup = ?',$idGroup)
		->where('sc_date = ?',$date)
		->where('sc_start_time = ?',$start)
		->where('sc_end_time = ?',$stop);
	...
	public function GetGroupScheduleByLecturer($idlec,$idsemester) {
		...
		->join(array('cg'=>'tbl_course_tagging_group_minor'),'cg.IdCourseTaggingGroupMinor=s.idGroup',array('GroupName'))
