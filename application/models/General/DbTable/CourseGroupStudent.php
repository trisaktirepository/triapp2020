...
class App_Model_General_DbTable_CourseGroupStudent extends Zend_Db_Table_Abstract {
	...
	public function getTotalStudent($idGroup){
		...
		$db = Zend_Db_Table::getDefaultAdapter();		
		 ...
		 $select = $db ->select()		
					  ->from(array('gsm'=>'tbl_studentregsubjects'))
...
public function getStudentMarkDistribution($course_id,$semester=null) {
    	
    	
    	$db = Zend_Db_Table::getDefaultAdapter();
    
    	$select = $db ->select()
    	->from(array('a'=>'tbl_studentregsubjects'),array('IdStudentRegSubjects'))
    	->where('a.IdCourseTaggingGroup = ?',$course_id);
    	...
    	if ($row) {
	    	$select = $db ->select()
	    	->from(array('sme'=>'tbl_student_marks_entry'),array('IdMarksDistributionMaster'))
	    	->join(array('m'=>'tbl_marksdistributionmaster'),'m.IdMarksDistributionMaster=sme.IdMarksDistributionMaster',array('IdBranch'))
	    	->where('sme.IdStudentRegSubjects = ?',$row['IdStudentRegSubjects'])
	    	
	    	->group('sme.IdMarksDistributionMaster');
	    	$row=$db->fetchAll($select);
    	}
    	return  $row;
