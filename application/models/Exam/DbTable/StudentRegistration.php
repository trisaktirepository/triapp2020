<?php 
class App_Model_Exam_DbTable_StudentRegistration extends Zend_Db_Table_Abstract
{
	...
	public function getStudentAttendExambyCourseGroupAllComponent($idProgram,$idGroup,$idSubject,$idSemester,$component,$formData=null,$idBranch=null){
		...
		->where('ct.ProgramCreator = ?',$idProgram)
		...
		foreach($rows as $key=>$student){
			...
			foreach ($component as $item) {
				...
				$examGroupDb =new App_Model_Exam_DbTable_ExamGroupStudent();
				...
				if(is_array($exam_group)){
					...
					//get exam attendance
					$examAttendanceDb = new App_Model_Exam_DbTable_ExamGroupAttendance();
