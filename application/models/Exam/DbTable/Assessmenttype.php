...
class App_Model_Exam_DbTable_Assessmenttype extends Zend_Db_Table_Abstract { //Model Class for Users Details
	...
	public function getdropdownforasseementtype(){
		$lstrSelect = $this->lobjDbAdpt->select()
		->from(array('a' => 'tbl_examination_assessment_type'), array('key' => 'a.IdExaminationAssessmentType', 'value' => 'a.DescriptionDefaultlang'))
		->order('a.DescriptionDefaultlang');
		$larrResult = $this->lobjDbAdpt->fetchAll($lstrSelect);
		return $larrResult;
	}
	
	public function getAssessmentOBEByProgram($programid,$idsubject=null,$idlandscape=null){
		 
			$lstrSelect = $this->lobjDbAdpt->select()
			 
			->from(array('a' => 'tbl_examination_assessment_type'), array('key' => 'a.IdExaminationAssessmentType', 'value' => 'a.DescriptionDefaultlang','Percentage'=>'sum(b.Percentage)'))
			->join(array('b'=>'tbl_rps_assessment'),'a.IdExaminationAssessmentType=b.idassessment_type',array())
			->join(array('c'=>'tbl_RPS'),'c.idsession=b.idsession',array())
			->join(array('d'=>'tbl_rps_head'),'c.idRps=d.idRps',array())
			->join(array('e'=>'tbl_landscape'),'e.idlandscape=d.idlandscape',array())
			->where('e.IdProgram=?',$programid)
			->where('d.IdSubject=?',$idsubject)
			->where('d.IdLandscape=?',$idlandscape)
			->group('a.IdExaminationAssessmentType');
			$larrResult = $this->lobjDbAdpt->fetchAll($lstrSelect);
	...
	public function getdropdownforasseementtypeByProgram($programid,$idsubject=null,$idlandscape=null){
		...
		if ($idsubject!=null) {
			...
			if (!$larrResult) {
				$lstrSelect = $this->lobjDbAdpt->select()
				->from(array('a' => 'tbl_examination_assessment_type'), array('key' => 'a.IdExaminationAssessmentType', 'value' => 'a.DescriptionDefaultlang'))
				->join(array('b'=>'tbl_examination_assessment_type_program'),'a.IdExaminationAssessmentType=b.IdAssessment',array())
				->where('b.IdProgram=?',$programid)
				->order('a.DescriptionDefaultlang');
				$larrResult = $this->lobjDbAdpt->fetchAll($lstrSelect);
				foreach ($larrResult as $key=>$value) {
					$larrResult[$key]['Percentage']=0;
				}
			} else {
				foreach ($larrResult as $key=>$value) {
					$larrResult[$key]['value']=$value['value'].' ('.$value['Percentage'].'%)';
				}
			}
		} else {
			$lstrSelect = $this->lobjDbAdpt->select()
