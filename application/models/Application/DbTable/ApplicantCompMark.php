...
class App_Model_Application_DbTable_ApplicantCompMark extends Zend_Db_Table_Abstract {
	...
	public function getCompMark($txnId,$compId=null){
		$db = Zend_Db_Table::getDefaultAdapter();
		
		$sql = $db->select()
			->from(array('a'=>$this->_name),array('apcm_mark'))
			->where('a.apcm_at_trans_id=?',$txnId);

		if ($compId!=null) $sql->where('a.apcm_apd_id=?',$compId);
	...
	public function getCompMarkByTxnIdCompId($txnId,$compId=null){
		$db = Zend_Db_Table::getDefaultAdapter();
