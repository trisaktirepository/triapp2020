...
class App_Model_Application_DbTable_ApplicantPtestDetail extends Zend_Db_Table_Abstract
{
	...
	public function getPtestDetail($transaction_id){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
	     ...
	     $select = $db ->select()
					...
					->where('b.apt_at_trans_id =?', $transaction_id);										
	...
	public function getActiveTest($transid,$date,$time){
		...
		$select = $db ->select()
		->from(array('a'=>$this->_name),array('app_comp_code','time_start','time_stop','timerange'=>'DATEDIFF(time_stop,time_start)'))
	...
	public function isIn($aptid,$compcode){
