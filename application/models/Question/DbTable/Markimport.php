<?php 
class App_Model_Question_DbTable_Markimport extends Zend_Db_Table_Abstract
{
	...
	public function getMark(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		 ...
		 echo $select = $db->select()
		             ->from(array('a'=>'mark_manual_20120728'))	;
//		             exit;
	...
	public function checkApplication($icno,$course, $examcenter,$idSchedule){
		
		$db = Zend_Db_Table::getDefaultAdapter();
		
		 $select = $db->select()
		             ->from(array('s'=>'r015_student'))	
		             ->joinLeft(array('q'=>'r016_registrationdetails'),'q.idApplication = s.ID',array('*'))
					 ->where('s.ARD_IC = "'.$icno.'"')
					 ->where('q.idCourse = '.$course)
					 ->where('q.idVenue = '.$examcenter)
					 ->where('q.idSchedule = '.$idSchedule);
