<?php
class App_Model_Record_DbTable_YudiciumPrerequisite extends Zend_Db_Table_Abstract {
	/**
	 * The default table name 
	 */
	protected $_name = 'tbl_yudicium_prerequisite';
	protected $_primary = "IdPreYudisium";
		
	public function getData($idprogram,$idmajor,$idsemester){
		$db = Zend_Db_Table::getDefaultAdapter();
		 
			if ($idsemester!=null) {
				$selectData = $db->select()
				->from(array('tbl_semestermaster'))
				->where('IdSemesterMaster=?',$idsemester);
				$sem=$db->fetchRow($selectData);
				$selectData = $db->select()
				->from(array('yp'=>$this->_name))
				->join(array('pr'=>'tbl_program'), 'pr.IdProgram = yp.IdProgram', array('program_name'=>'ArabicName'))
				->join(array('s'=>'tbl_semestermaster'),'s.IdSemesterMaster=yp.IdSemesterMain')
				->where('yp.IdProgram=?',$idprogram)
				->where('s.SemesterMainStartDate <= ?',$sem['SemesterMainStartDate'])
				->order('s.SemesterMainStartDate');
			} else {
				$selectData = $db->select()
				->from(array('yp'=>$this->_name))
				->join(array('pr'=>'tbl_program'), 'pr.IdProgram = yp.IdProgram', array('program_name'=>'ArabicName'))
