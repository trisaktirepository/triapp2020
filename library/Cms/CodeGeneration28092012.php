	...
	public function RegistrationIdGenration($iduniversity,$configArray,$studentdetails=NULL){
	  ...
	  if($genderID=='1') { $genderData = 'F';  } else {  $genderData = 'M'; }
		...
		switch(trim($lstrresetconfig)) {
			case "":
				$seqno = "00".($studentList + 1);
				break;
			case "program":
				$seqno = "00".$this->lobjstudentgenerated->fngetseqno("program",$programID,"");
				break;
			case "intake":
				$seqno = "00".$this->lobjstudentgenerated->fngetseqno("intake","",$intakeID);
				break;
			case "progintake":
				$seqno = "00".$this->lobjstudentgenerated->fngetseqno("progintake",$programID,$intakeID);
				break;
			case "year":
				$seqno = "00".$this->lobjstudentgenerated->fngetseqno("year");
				break;
		}
